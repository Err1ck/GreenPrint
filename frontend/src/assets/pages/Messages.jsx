...
function Messages() {
    ...
    const fetchConversations = async (userId) => {
        try {
            ...
            const response = await fetch(`http://127.0.0.1:8000/api/messages/conversations/${userId}`, {
                headers: {
    ...
    const fetchMessages = async (conversationId) => {
        try {
            const token = localStorage.getItem("token");
            const response = await fetch(`http://127.0.0.1:8000/api/messages/conversation/${conversationId}`, {
                headers: {
                    "Authorization": `Bearer ${token}`
                }
            });
